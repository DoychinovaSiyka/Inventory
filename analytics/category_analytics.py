...
def build_category_tree(categories: List[Category]) -> List[dict]:
    """ Изгражда йерархична структура (дърво) от плосък списък с категории. Връща списък от речници,
    подходящ за визуализация в менюта."""
    tree = []
    #  Намирам коренните категории (тези без родител)
    root_categories = [c for c in categories if c.parent_id is None]
    # За всяка коренна категория, намирам нейните деца рекурсивно
    for root in root_categories:
        # Добавям корена (Ниво 0)
        tree.append({"category": root, "level": 0})
        # Викам помощната функция за децата му
