...
class InvoiceController:
    ...
    def add(self, invoice_data: dict, user_id: str) -> Invoice:
        """Ръчно добавяне на фактура след пълна валидация."""
        InvoiceValidator.validate_all(**invoice_data)

        # Моделът сам генерира ID и дати
        invoice = Invoice(invoice_id=None,**invoice_data)
    ...
    def create_from_movement(self, movement, product, customer: str, user_id: str) -> Invoice:
        """Автоматично създаване на фактура от OUT движение."""
        ...
        # Моделът сам генерира ID и дати
        invoice = Invoice(invoice_id=None, movement_id=movement.movement_id,
                          product=product.name, quantity=qty, unit=movement.unit,
