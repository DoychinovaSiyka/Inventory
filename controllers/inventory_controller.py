...
class InventoryController:
    ...
    def rebuild_inventory_from_movements(self, movements: List) -> None:
        ...
        for m in sorted_moves:
            ...
            if m_type == "IN" and m.location_id:
                ...
                locs[m.location_id] = max(0.0, current - qty)

            elif m_type == "MOVE":
                if m.from_location_id:
                    current = locs.get(m.from_location_id, 0.0)
                    locs[m.from_location_id] = max(0.0, current - qty)
    ...
    def get_total_inventory_value_fifo(self, movement_controller) -> float:
        ...
        for pid in self.data.get("products", {}):
            ...
            prod_moves = [m for m in all_moves if str(m.product_id) == pid]
            prod_moves = sorted(prod_moves, key=lambda x: x.date)
            ...
            # Стойността на това, което е останало
            for b in batches:
