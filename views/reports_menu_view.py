...
class ReportsView:
    ...
    def filter_movements_by_client(self, _):
        client = input("Въведете клиент (Enter за всички): ").strip()
        result = self.controller.report_sales()
        ...
        for m in result.data:
            ...
            rows.append([
                ...
                m.get("date", "-"),
                m.get("client", "Неизвестен"),
                m.get("product", "-"),
                qty_display,
                m.get("total_price", "-"),
                m.get("status", "АКТИВНА")
            ])
